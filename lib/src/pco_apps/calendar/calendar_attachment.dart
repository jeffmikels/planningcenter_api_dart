...
enum PcoCalendarAttachmentOrder { contentType, createdAt, description, fileSize, name, updatedAt }
...
class PcoCalendarAttachmentQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCalendarAttachmentOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCalendarAttachmentFilter, String> _filterMap = {
    
  };
  static String filterString(PcoCalendarAttachmentFilter filter) => _filterMap[filter]!;
  ...
  PcoCalendarAttachmentQuery({
    ...
    if (whereContentType != null) where.add(PlanningCenterApiWhere.parse('content_type', whereContentType));
...
class PcoCalendarAttachment extends PcoResource {
  ...
  static Future<PcoCollection<PcoCalendarAttachment>> get( {String? id, PcoCalendarAttachmentQuery? query, bool includeEvent = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarAttachment>> getFromEvent(String eventId, {String? id, PcoCalendarAttachmentQuery? query, bool includeEvent = false,}) async {
    query ??= PcoCalendarAttachmentQuery();
