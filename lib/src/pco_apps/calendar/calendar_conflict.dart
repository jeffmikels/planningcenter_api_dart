...
class PcoCalendarConflictQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCalendarConflictOrder order, {bool reverse = false}) =>
  ...
  static String filterString(PcoCalendarConflictFilter filter) => _filterMap[filter]!;
...
class PcoCalendarConflict extends PcoResource {
  ...
  static Future<PcoCollection<PcoCalendarConflict>> get( {String? id, PcoCalendarConflictQuery? query, bool includeAll = false, bool includeResolvedBy = false, bool includeResource = false, bool includeWinner = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarConflict>> getFromEvent(String eventId, {String? id, PcoCalendarConflictQuery? query, bool includeAll = false, bool includeResolvedBy = false, bool includeResource = false, bool includeWinner = false,}) async {
    query ??= PcoCalendarConflictQuery();
    if (includeAll) query.include.addAll(PcoCalendarConflict.canInclude);
    if (includeResolvedBy) query.include.add('resolved_by');
    if (includeResource) query.include.add('resource');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarConflict>> getFromResource(String resourceId, {String? id, PcoCalendarConflictQuery? query, bool includeAll = false, bool includeResolvedBy = false, bool includeResource = false, bool includeWinner = false,}) async {
    query ??= PcoCalendarConflictQuery();
    if (includeAll) query.include.addAll(PcoCalendarConflict.canInclude);
    if (includeResolvedBy) query.include.add('resolved_by');
    if (includeResource) query.include.add('resource');
  ...
  Future<PcoCollection<PcoCalendarPerson>> getResolvedBy({PcoCalendarPersonQuery? query}) async {
  ...
  Future<PcoCollection<PcoCalendarEvent>> getWinner({PcoCalendarEventQuery? query}) async {
