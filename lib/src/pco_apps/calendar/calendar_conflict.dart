/// =========================================================================
/// AUTO-GENERATED FILE CREATED ON 2022-07-28T11:29:17.783275
...
/// - Manually create an object using the `PcoCalendarConflict.manual()` constructor.
...
class PcoCalendarConflict extends PcoResource {
  ...
  List<T> includedResolvedBy<T extends PcoResource>() => (relationships['resolved_by'] as List?)?.cast<T>() ?? [];
  PcoCalendarResource? get includedResource => _firstOrNull<PcoCalendarResource>(relationships['resource']);
  /// The code generator could not automatically determine the resource type of this relationship.
  /// For type safe code, you should specify it here.
  List<T> includedWinner<T extends PcoResource>() => (relationships['winner'] as List?)?.cast<T>() ?? [];

  // Class Constructors
  PcoCalendarConflict.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoCalendarConflict({String? id, DateTime? createdAt, String? note, DateTime? resolvedAt, DateTime? updatedAt, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
    var obj = PcoCalendarConflict.empty();
    obj._id = id;
    if (createdAt != null) obj._attributes['created_at'] = createdAt.toIso8601String();
    if (note != null) obj._attributes['note'] = note;
    if (resolvedAt != null) obj._attributes['resolved_at'] = resolvedAt.toIso8601String();
  ...
  /// - `resolved`
  /// - `unresolved`
  static Future<PcoCollection<PcoCalendarConflict>> get( {String? id, PlanningCenterApiQuery? query, bool includeAll = false, bool includeResolvedBy = false, bool includeResource = false, bool includeWinner = false,}) async {
  ...
  static Future<PcoCollection<PcoCalendarConflict>> getFromEvent(String eventId, {String? id, PlanningCenterApiQuery? query, bool includeAll = false, bool includeResolvedBy = false, bool includeResource = false, bool includeWinner = false,}) async {
    query ??= PlanningCenterApiQuery();
    if (includeAll) query.include.addAll(PcoCalendarConflict.canInclude);
    if (includeResolvedBy) query.include.add('resolved_by');
    if (includeResource) query.include.add('resource');
  ...
  static Future<PcoCollection<PcoCalendarConflict>> getFromResource(String resourceId, {String? id, PlanningCenterApiQuery? query, bool includeAll = false, bool includeResolvedBy = false, bool includeResource = false, bool includeWinner = false,}) async {
    query ??= PlanningCenterApiQuery();
    if (includeAll) query.include.addAll(PcoCalendarConflict.canInclude);
    if (includeResolvedBy) query.include.add('resolved_by');
    if (includeResource) query.include.add('resource');
  ...
  Future<PcoCollection<PcoCalendarPerson>> getResolvedBy({PlanningCenterApiQuery? query, }) async {
    query ??= PlanningCenterApiQuery();
  ...
  Future<PcoCollection<PcoCalendarEvent>> getWinner({PlanningCenterApiQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
    query ??= PlanningCenterApiQuery();
    if (includeAll) query.include.addAll(PcoCalendarConflict.canInclude);
