...
class PcoCalendarEventQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCalendarEventOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCalendarEventFilter, String> _filterMap = {
    PcoCalendarEventFilter.future: 'future',
  };
  static String filterString(PcoCalendarEventFilter filter) => _filterMap[filter]!;
  ...
  PcoCalendarEventQuery({
    ...
    if (includeAll || includeAttachments) include.add('attachments');
    ...
    if (wherePercentRejected != null) where.add(PlanningCenterApiWhere.parse('percent_rejected', wherePercentRejected));
    if (whereUpdatedAt != null) where.add(PlanningCenterApiWhere.parse('updated_at', whereUpdatedAt));
    if (whereVisibleInChurchCenter != null) where.add(PlanningCenterApiWhere.parse('visible_in_church_center', whereVisibleInChurchCenter));}
}
...
class PcoCalendarEvent extends PcoResource {
  ...
  static Future<PcoCollection<PcoCalendarEvent>> get( {String? id, PcoCalendarEventQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarEvent>> getFromAttachment(String attachmentId, {PcoCalendarEventQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
    query ??= PcoCalendarEventQuery();
    if (includeAll) query.include.addAll(PcoCalendarEvent.canInclude);
    if (includeAttachments) query.include.add('attachments');
    if (includeOwner) query.include.add('owner');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarEvent>> getWinnerFromConflict(String conflictId, {PcoCalendarEventQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
    query ??= PcoCalendarEventQuery();
    if (includeAll) query.include.addAll(PcoCalendarEvent.canInclude);
    if (includeAttachments) query.include.add('attachments');
    if (includeOwner) query.include.add('owner');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarEvent>> getFromEventInstance(String eventInstanceId, {PcoCalendarEventQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
    query ??= PcoCalendarEventQuery();
    if (includeAll) query.include.addAll(PcoCalendarEvent.canInclude);
    if (includeAttachments) query.include.add('attachments');
    if (includeOwner) query.include.add('owner');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarEvent>> getFromEventResourceRequest(String eventResourceRequestId, {PcoCalendarEventQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
    query ??= PcoCalendarEventQuery();
    if (includeAll) query.include.addAll(PcoCalendarEvent.canInclude);
    if (includeAttachments) query.include.add('attachments');
    if (includeOwner) query.include.add('owner');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarEvent>> getFromEventInstanceAndEventTime(String eventInstanceId,String eventTimeId, {PcoCalendarEventQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
    query ??= PcoCalendarEventQuery();
    if (includeAll) query.include.addAll(PcoCalendarEvent.canInclude);
    if (includeAttachments) query.include.add('attachments');
    if (includeOwner) query.include.add('owner');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarEvent>> getFromTag(String tagId, {String? id, PcoCalendarEventQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
    query ??= PcoCalendarEventQuery();
    if (includeAll) query.include.addAll(PcoCalendarEvent.canInclude);
    if (includeAttachments) query.include.add('attachments');
    if (includeOwner) query.include.add('owner');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarEvent>> getFromTagGroup(String tagGroupId, {String? id, PcoCalendarEventQuery? query, bool includeAll = false, bool includeAttachments = false, bool includeOwner = false, bool includeTags = false,}) async {
    query ??= PcoCalendarEventQuery();
    if (includeAll) query.include.addAll(PcoCalendarEvent.canInclude);
    if (includeAttachments) query.include.add('attachments');
    if (includeOwner) query.include.add('owner');
  ...
  Future<PcoCollection<PcoCalendarAttachment>> getAttachments({PcoCalendarAttachmentQuery? query}) async {
  ...
  /// Available Query Filters:
  /// - `future`
  Future<PcoCollection<PcoCalendarEventInstance>> getEventInstances({PcoCalendarEventInstanceQuery? query}) async {
  ...
  Future<PcoCollection<PcoCalendarFeed>> getFeed({PcoCalendarFeedQuery? query}) async {
  ...
  Future<PcoCollection<PcoCalendarPerson>> getOwner({PcoCalendarPersonQuery? query}) async {
  ...
  Future<PcoCollection<PcoCalendarTag>> getTags({PcoCalendarTagQuery? query}) async {
