...
class PcoCalendarFeedQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCalendarFeedOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCalendarFeedFilter, String> _filterMap = {
    
  };
  static String filterString(PcoCalendarFeedFilter filter) => _filterMap[filter]!;
  ...
  PcoCalendarFeedQuery({
    ...
    if (whereFeedType != null) where.add(PlanningCenterApiWhere.parse('feed_type', whereFeedType));
...
class PcoCalendarFeed extends PcoResource {
  ...
  String get defaultChurchCenterVisibility => _attributes[kDefaultChurchCenterVisibility] ?? '';
  ...
  ///
  /// pass `null` to remove key from attributes
  set eventOwnerId(String? x) => (x == null) ? _attributes.remove(kEventOwnerId) : _attributes[kEventOwnerId] = x;
  String get eventOwnerId => _attributes[kEventOwnerId] ?? '';

  // Class Constructors
  PcoCalendarFeed.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoCalendarFeed({String? id, String? defaultChurchCenterVisibility, String? feedType, String? name, DateTime? importedAt, bool? isCanDelete, String? eventOwnerId, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
    var obj = PcoCalendarFeed.empty();
    obj._id = id;
    if (defaultChurchCenterVisibility != null) obj._attributes['default_church_center_visibility'] = defaultChurchCenterVisibility;
    if (feedType != null) obj._attributes['feed_type'] = feedType;
    if (name != null) obj._attributes['name'] = name;
    if (importedAt != null) obj._attributes['imported_at'] = importedAt.toIso8601String();
  ...
  static Future<PcoCollection<PcoCalendarFeed>> get( {String? id, PcoCalendarFeedQuery? query, }) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarFeed>> getFromEvent(String eventId, {PcoCalendarFeedQuery? query, }) async {
    query ??= PcoCalendarFeedQuery();
