...
class PcoCalendarOrganization extends PcoResource {
  ...
  /// Will get a [PcoCollection] of [PcoCalendarOrganization] objects (expecting many)
  ...
  /// Will get a single [PcoCalendarOrganization] object
  /// using a path like this: `/calendar/v2/people/$personId/organization/[id]`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCalendarOrganization?> getSingleFromPerson(
    String personId,
    String id, {
    PcoCalendarOrganizationQuery? query,
  }) async {
    ...
    var url = '/calendar/v2/people/$personId/organization/$id';
    var retval = await PcoCollection.fromApiCall<PcoCalendarOrganization>(url,
        query: query, apiVersion: kApiVersion);
    return retval.items.isEmpty ? null : retval.items.first;
  }

  /// Will get a [PcoCollection] containing ALL [PcoCalendarOrganization] objects (expecting many)
  /// using a path like this: `/calendar/v2/people/$personId/organization`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  ...
  static Future<PcoCollection<PcoCalendarOrganization>> getAllFromPerson(
    String personId, {
    String? id,
    PcoCalendarOrganizationQuery? query,
  }) async {
