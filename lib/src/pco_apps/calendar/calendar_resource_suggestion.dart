...
class PcoCalendarResourceSuggestionQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCalendarResourceSuggestionOrder order,
          {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCalendarResourceSuggestionFilter, String> _filterMap = {};
  static String filterString(PcoCalendarResourceSuggestionFilter filter) =>
      _filterMap[filter]!;
...
class PcoCalendarResourceSuggestion extends PcoResource {
  ...
  factory PcoCalendarResourceSuggestion(
      {String? id,
      DateTime? createdAt,
      int? quantity,
      DateTime? updatedAt,
  ...
  /// using a path like this: `/calendar/v2/room_setups/$roomSetupId/resource_suggestions/[id]`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCalendarResourceSuggestion?> getSingleFromRoomSetup(
    String roomSetupId,
    String id, {
    PcoCalendarResourceSuggestionQuery? query,
    bool includeResource = false,
    ...
    var retval = await PcoCollection.fromApiCall<PcoCalendarResourceSuggestion>(
        url,
        query: query,
  ...
  /// Will get a [PcoCollection] containing ALL [PcoCalendarResourceSuggestion] objects (expecting many)
  /// using a path like this: `/calendar/v2/room_setups/$roomSetupId/resource_suggestions`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  ...
  static Future<PcoCollection<PcoCalendarResourceSuggestion>>
      getAllFromRoomSetup(
    String roomSetupId, {
    String? id,
    PcoCalendarResourceSuggestionQuery? query,
