...
class PcoCalendarTagQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCalendarTagOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCalendarTagFilter, String> _filterMap = {
    PcoCalendarTagFilter.individual: 'individual',
  };
  static String filterString(PcoCalendarTagFilter filter) => _filterMap[filter]!;
  ...
  PcoCalendarTagQuery({
    ...
    if (whereColor != null) where.add(PlanningCenterApiWhere.parse('color', whereColor));
    ...
    if (wherePosition != null) where.add(PlanningCenterApiWhere.parse('position', wherePosition));
...
class PcoCalendarTag extends PcoResource {
  ...
  static Future<PcoCollection<PcoCalendarTag>> get( {String? id, PcoCalendarTagQuery? query, bool includeTagGroup = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarTag>> getFromEventInstance(String eventInstanceId, {String? id, PcoCalendarTagQuery? query, bool includeTagGroup = false,}) async {
    query ??= PcoCalendarTagQuery();
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarTag>> getFromEvent(String eventId, {String? id, PcoCalendarTagQuery? query, bool includeTagGroup = false,}) async {
    query ??= PcoCalendarTagQuery();
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarTag>> getFromTagGroup(String tagGroupId, {String? id, PcoCalendarTagQuery? query, bool includeTagGroup = false,}) async {
    query ??= PcoCalendarTagQuery();
  ...
  Future<PcoCollection<PcoCalendarEvent>> getEvents({PcoCalendarEventQuery? query}) async {
  ...
  Future<PcoCollection<PcoCalendarTagGroup>> getTagGroup({PcoCalendarTagGroupQuery? query}) async {
