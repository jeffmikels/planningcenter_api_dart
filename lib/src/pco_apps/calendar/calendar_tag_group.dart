...
class PcoCalendarTagGroupQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCalendarTagGroupOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCalendarTagGroupFilter, String> _filterMap = {
    PcoCalendarTagGroupFilter.required: 'required',
  };
  static String filterString(PcoCalendarTagGroupFilter filter) => _filterMap[filter]!;
...
class PcoCalendarTagGroup extends PcoResource {
  ...
  List<PcoCalendarEvent> get includedEvents => (relationships['events'] as List?)?.cast<PcoCalendarEvent>() ?? [];
  List<PcoCalendarTag> get includedTags => (relationships['tags'] as List?)?.cast<PcoCalendarTag>() ?? [];

  // Class Constructors
  PcoCalendarTagGroup.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoCalendarTagGroup({String? id, DateTime? createdAt, String? name, DateTime? updatedAt, bool? isRequired, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
  ...
  static Future<PcoCollection<PcoCalendarTagGroup>> get( {String? id, PcoCalendarTagGroupQuery? query, bool includeAll = false, bool includeEvents = false, bool includeTags = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCalendarTagGroup>> getFromTag(String tagId, {String? id, PcoCalendarTagGroupQuery? query, bool includeAll = false, bool includeEvents = false, bool includeTags = false,}) async {
    query ??= PcoCalendarTagGroupQuery();
    if (includeAll) query.include.addAll(PcoCalendarTagGroup.canInclude);
    if (includeEvents) query.include.add('events');
    if (includeTags) query.include.add('tags');
