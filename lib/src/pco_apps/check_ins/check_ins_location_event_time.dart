...
class PcoCheckInsLocationEventTimeQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCheckInsLocationEventTimeOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCheckInsLocationEventTimeFilter, String> _filterMap = {
    
  };
  static String filterString(PcoCheckInsLocationEventTimeFilter filter) => _filterMap[filter]!;
...
class PcoCheckInsLocationEventTime extends PcoResource {
  ...
  static Future<PcoCollection<PcoCheckInsLocationEventTime>> getFromEventTime(String eventTimeId, {String? id, PcoCheckInsLocationEventTimeQuery? query, bool includeAll = false, bool includeEventTime = false, bool includeLocation = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCheckInsLocationEventTime>> getFromCheckInAndLocation(String checkInId,String locationId, {String? id, PcoCheckInsLocationEventTimeQuery? query, bool includeAll = false, bool includeEventTime = false, bool includeLocation = false,}) async {
    query ??= PcoCheckInsLocationEventTimeQuery();
    if (includeAll) query.include.addAll(PcoCheckInsLocationEventTime.canInclude);
    if (includeEventTime) query.include.add('event_time');
    if (includeLocation) query.include.add('location');
