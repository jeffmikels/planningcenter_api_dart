/// =========================================================================
/// AUTO-GENERATED FILE CREATED ON 2022-10-20T17:42:22.787715
...
class PcoCheckInsLocationLabelQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCheckInsLocationLabelOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCheckInsLocationLabelFilter, String> _filterMap = {
    
  };
  static String filterString(PcoCheckInsLocationLabelFilter filter) => _filterMap[filter]!;
...
class PcoCheckInsLocationLabel extends PcoResource {
  ...
  PcoCheckInsLabel? get includedLabel => _firstOrNull<PcoCheckInsLabel>(relationships['label']);
  PcoCheckInsLocation? get includedLocation => _firstOrNull<PcoCheckInsLocation>(relationships['location']);

  // Class Constructors
  PcoCheckInsLocationLabel.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoCheckInsLocationLabel({String? id, int? quantity, bool? isForRegular, bool? isForGuest, bool? isForVolunteer, DateTime? createdAt, DateTime? updatedAt, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
    ...
    if (isForVolunteer != null) obj._attributes['for_volunteer'] = isForVolunteer;
  ...
  static Future<PcoCollection<PcoCheckInsLocationLabel>> getFromLabel(String labelId, {String? id, PcoCheckInsLocationLabelQuery? query, bool includeAll = false, bool includeLabel = false, bool includeLocation = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCheckInsLocationLabel>> getFromCheckInAndLocation(String checkInId,String locationId, {String? id, PcoCheckInsLocationLabelQuery? query, bool includeAll = false, bool includeLabel = false, bool includeLocation = false,}) async {
    query ??= PcoCheckInsLocationLabelQuery();
    if (includeAll) query.include.addAll(PcoCheckInsLocationLabel.canInclude);
    if (includeLabel) query.include.add('label');
    if (includeLocation) query.include.add('location');
  ...
  Future<PcoCollection<PcoCheckInsLabel>> getLabel({PcoCheckInsLabelQuery? query}) async {
