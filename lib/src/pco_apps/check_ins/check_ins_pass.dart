...
class PcoCheckInsPassQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoCheckInsPassOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoCheckInsPassFilter, String> _filterMap = {
    
  };
  static String filterString(PcoCheckInsPassFilter filter) => _filterMap[filter]!;
  ...
  PcoCheckInsPassQuery({
    ...
    if (whereCode != null) where.add(PlanningCenterApiWhere.parse('code', whereCode));}
}
...
class PcoCheckInsPass extends PcoResource {
  ...
  // Class Constructors
  PcoCheckInsPass.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  static Future<PcoCollection<PcoCheckInsPass>> get( {String? id, PcoCheckInsPassQuery? query, bool includePerson = false,}) async {
    query ??= PcoCheckInsPassQuery();
    
    if (includePerson) query.include.add('person');
    var url = '/check-ins/v2/passes';
    if (id != null) url += '/$id';
    return PcoCollection.fromApiCall<PcoCheckInsPass>(url, query: query, apiVersion:kApiVersion);
  }
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoCheckInsPass>> getFromPerson(String personId, {String? id, PcoCheckInsPassQuery? query, bool includePerson = false,}) async {
    query ??= PcoCheckInsPassQuery();
