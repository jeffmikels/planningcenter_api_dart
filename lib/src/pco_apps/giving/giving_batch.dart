...
class PcoGivingBatchQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoGivingBatchOrder order, {bool reverse = false}) =>
  ...
  static String filterString(PcoGivingBatchFilter filter) => _filterMap[filter]!;
...
class PcoGivingBatch extends PcoResource {
  ...
  static Future<PcoCollection<PcoGivingBatch>> get( {String? id, PcoGivingBatchQuery? query, bool includeAll = false, bool includeBatchGroup = false, bool includeOwner = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoGivingBatch>> getFromBatchGroup(String batchGroupId, {String? id, PcoGivingBatchQuery? query, bool includeAll = false, bool includeBatchGroup = false, bool includeOwner = false,}) async {
    query ??= PcoGivingBatchQuery();
    if (includeAll) query.include.addAll(PcoGivingBatch.canInclude);
    if (includeBatchGroup) query.include.add('batch_group');
    if (includeOwner) query.include.add('owner');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoGivingBatch>> getFromPerson(String personId, {String? id, PcoGivingBatchQuery? query, bool includeAll = false, bool includeBatchGroup = false, bool includeOwner = false,}) async {
    query ??= PcoGivingBatchQuery();
    if (includeAll) query.include.addAll(PcoGivingBatch.canInclude);
    if (includeBatchGroup) query.include.add('batch_group');
    if (includeOwner) query.include.add('owner');
  ...
  Future<PcoCollection<PcoGivingBatchGroup>> getBatchGroup({PcoGivingBatchGroupQuery? query}) async {
  ...
  Future<PcoCollection<PcoGivingDonation>> getDonations({PcoGivingDonationQuery? query}) async {
