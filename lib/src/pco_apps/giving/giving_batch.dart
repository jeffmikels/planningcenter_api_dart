/// =========================================================================
/// AUTO-GENERATED FILE CREATED ON 2022-07-28T11:29:17.812084
...
/// - Manually create an object using the `PcoGivingBatch.manual()` constructor.
...
class PcoGivingBatch extends PcoResource {
  ...
  PcoGivingBatchGroup? get includedBatchGroup => _firstOrNull<PcoGivingBatchGroup>(relationships['batch_group']);
  /// The code generator could not automatically determine the resource type of this relationship.
  /// For type safe code, you should specify it here.
  List<T> includedOwner<T extends PcoResource>() => (relationships['owner'] as List?)?.cast<T>() ?? [];

  // Class Constructors
  PcoGivingBatch.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoGivingBatch({String? id, DateTime? createdAt, DateTime? updatedAt, DateTime? committedAt, String? description, int? donationsCount, int? totalCents, String? totalCurrency, String? status, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
    ...
    if (committedAt != null) obj._attributes['committed_at'] = committedAt.toIso8601String();
    if (description != null) obj._attributes['description'] = description;
    if (donationsCount != null) obj._attributes['donations_count'] = donationsCount;
  ...
  /// - `committed`
  /// - `in_progress`
  static Future<PcoCollection<PcoGivingBatch>> get( {String? id, PlanningCenterApiQuery? query, bool includeAll = false, bool includeBatchGroup = false, bool includeOwner = false,}) async {
  ...
  /// - `committed`
  /// - `in_progress`
  static Future<PcoCollection<PcoGivingBatch>> getFromBatchGroup(String batchGroupId, {String? id, PlanningCenterApiQuery? query, bool includeAll = false, bool includeBatchGroup = false, bool includeOwner = false,}) async {
    query ??= PlanningCenterApiQuery();
    if (includeAll) query.include.addAll(PcoGivingBatch.canInclude);
    if (includeBatchGroup) query.include.add('batch_group');
    if (includeOwner) query.include.add('owner');
  ...
  /// - `committed`
  /// - `in_progress`
  static Future<PcoCollection<PcoGivingBatch>> getFromPerson(String personId, {String? id, PlanningCenterApiQuery? query, bool includeAll = false, bool includeBatchGroup = false, bool includeOwner = false,}) async {
    query ??= PlanningCenterApiQuery();
    if (includeAll) query.include.addAll(PcoGivingBatch.canInclude);
    if (includeBatchGroup) query.include.add('batch_group');
    if (includeOwner) query.include.add('owner');
  ...
  Future<PcoCollection<PcoGivingBatchGroup>> getBatchGroup({PlanningCenterApiQuery? query, bool includeOwner = false,}) async {
    query ??= PlanningCenterApiQuery();
  ...
  Future<PcoCollection<PcoGivingDonation>> getDonations({PlanningCenterApiQuery? query, bool includeAll = false, bool includeDesignations = false, bool includeLabels = false, bool includeRefund = false,}) async {
    query ??= PlanningCenterApiQuery();
    if (includeAll) query.include.addAll(PcoGivingBatch.canInclude);
  ...
  Future<PcoCollection<PcoGivingPerson>> getOwner({PlanningCenterApiQuery? query, }) async {
    query ??= PlanningCenterApiQuery();
