...
class PcoGivingPledgeQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoGivingPledgeOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoGivingPledgeFilter, String> _filterMap = {
    
  };
  static String filterString(PcoGivingPledgeFilter filter) => _filterMap[filter]!;
...
class PcoGivingPledge extends PcoResource {
  ...
  static Future<PcoCollection<PcoGivingPledge>> getFromPerson(String personId, {String? id, PcoGivingPledgeQuery? query, bool includeAll = false, bool includeJointGiver = false, bool includePledgeCampaign = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoGivingPledge>> getFromPledgeCampaign(String pledgeCampaignId, {String? id, PcoGivingPledgeQuery? query, bool includeAll = false, bool includeJointGiver = false, bool includePledgeCampaign = false,}) async {
    query ??= PcoGivingPledgeQuery();
    if (includeAll) query.include.addAll(PcoGivingPledge.canInclude);
    if (includeJointGiver) query.include.add('joint_giver');
    if (includePledgeCampaign) query.include.add('pledge_campaign');
  ...
  Future<PcoCollection<PcoGivingPerson>> getJointGiver({PcoGivingPersonQuery? query}) async {
  ...
  Future<PcoCollection<PcoGivingPledgeCampaign>> getPledgeCampaign({PcoGivingPledgeCampaignQuery? query}) async {
