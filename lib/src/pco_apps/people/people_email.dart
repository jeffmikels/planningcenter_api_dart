...
class PcoPeopleEmailQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoPeopleEmailOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoPeopleEmailFilter, String> _filterMap = {
    
  };
  static String filterString(PcoPeopleEmailFilter filter) => _filterMap[filter]!;
  ...
  PcoPeopleEmailQuery({
    ...
    if (whereAddress != null) where.add(PlanningCenterApiWhere.parse('address', whereAddress));
    if (whereBlocked != null) where.add(PlanningCenterApiWhere.parse('blocked', whereBlocked));
...
class PcoPeopleEmail extends PcoResource {
  ...
  set address(String? x) => (x == null) ? _attributes.remove(kAddress) : _attributes[kAddress] = x;
  ///
  /// pass `null` to remove key from attributes
  ...
  // Class Constructors
  PcoPeopleEmail.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoPeopleEmail({required String personId, String? id, String? address, String? location, bool? isPrimary, DateTime? createdAt, DateTime? updatedAt, bool? isBlocked, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
    var obj = PcoPeopleEmail.empty();
    obj._id = id;
    obj._apiPathOverride = 'https://api.planningcenteronline.com/people/v2/people/$personId/emails';
  ...
  static Future<PcoCollection<PcoPeopleEmail>> get( {String? id, PcoPeopleEmailQuery? query, }) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleEmail>> getFromPerson(String personId, {String? id, PcoPeopleEmailQuery? query, }) async {
    query ??= PcoPeopleEmailQuery();
