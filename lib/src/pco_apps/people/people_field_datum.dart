...
/// - `value` -> `?order=value`
enum PcoPeopleFieldDatumOrder { file, fileContentType, fileName, fileSize, value }
...
class PcoPeopleFieldDatumQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoPeopleFieldDatumOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoPeopleFieldDatumFilter, String> _filterMap = {
    
  };
  static String filterString(PcoPeopleFieldDatumFilter filter) => _filterMap[filter]!;
  ...
  PcoPeopleFieldDatumQuery({
    ...
    if (whereFileContentType != null) where.add(PlanningCenterApiWhere.parse('file_content_type', whereFileContentType));
    if (whereFileName != null) where.add(PlanningCenterApiWhere.parse('file_name', whereFileName));
    if (whereFileSize != null) where.add(PlanningCenterApiWhere.parse('file_size', whereFileSize));
...
class PcoPeopleFieldDatum extends PcoResource {
  ...
  static Future<PcoCollection<PcoPeopleFieldDatum>> get( {String? id, PcoPeopleFieldDatumQuery? query, bool includeAll = false, bool includeFieldDefinition = false, bool includeFieldOption = false, bool includeTab = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleFieldDatum>> getFromPerson(String personId, {String? id, PcoPeopleFieldDatumQuery? query, bool includeAll = false, bool includeFieldDefinition = false, bool includeFieldOption = false, bool includeTab = false,}) async {
    query ??= PcoPeopleFieldDatumQuery();
    if (includeAll) query.include.addAll(PcoPeopleFieldDatum.canInclude);
    if (includeFieldDefinition) query.include.add('field_definition');
    if (includeFieldOption) query.include.add('field_option');
  ...
  Future<PcoCollection<PcoPeopleFieldDefinition>> getFieldDefinition({PcoPeopleFieldDefinitionQuery? query}) async {
  ...
  Future<PcoCollection<PcoPeopleFieldOption>> getFieldOption({PcoPeopleFieldOptionQuery? query}) async {
