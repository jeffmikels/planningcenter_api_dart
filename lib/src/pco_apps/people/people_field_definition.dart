...
/// - `slug` -> `?order=slug`
/// - `tabId` -> `?order=tab_id`
enum PcoPeopleFieldDefinitionOrder { config, dataType, deletedAt, name, sequence, slug, tabId }
...
class PcoPeopleFieldDefinitionQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoPeopleFieldDefinitionOrder order, {bool reverse = false}) =>
  ...
  static String filterString(PcoPeopleFieldDefinitionFilter filter) => _filterMap[filter]!;
  ...
  PcoPeopleFieldDefinitionQuery({
    ...
    if (whereConfig != null) where.add(PlanningCenterApiWhere.parse('config', whereConfig));
    if (whereDataType != null) where.add(PlanningCenterApiWhere.parse('data_type', whereDataType));
    ...
    if (whereSequence != null) where.add(PlanningCenterApiWhere.parse('sequence', whereSequence));
    if (whereSlug != null) where.add(PlanningCenterApiWhere.parse('slug', whereSlug));
    if (whereTabId != null) where.add(PlanningCenterApiWhere.parse('tab_id', whereTabId));
...
class PcoPeopleFieldDefinition extends PcoResource {
  ...
  set dataType(String? x) => (x == null) ? _attributes.remove(kDataType) : _attributes[kDataType] = x;
  ///
  /// pass `null` to remove key from attributes
  ...
  ///
  /// pass `null` to remove key from attributes
  set config(String? x) => (x == null) ? _attributes.remove(kConfig) : _attributes[kConfig] = x;
  ///
  /// pass `null` to remove key from attributes
  set deletedAt(DateTime? x) => (x == null) ? _attributes.remove(kDeletedAt) : _attributes[kDeletedAt] = x.toIso8601String();
  
  // typed getters for each relationship
  
  List<PcoPeopleFieldOption> get includedFieldOptions => (relationships['field_options'] as List?)?.cast<PcoPeopleFieldOption>() ?? [];
  PcoPeopleTab? get includedTab => _firstOrNull<PcoPeopleTab>(relationships['tab']);

  // Class Constructors
  PcoPeopleFieldDefinition.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoPeopleFieldDefinition({required String tabId, String? id, String? dataType, String? name, int? sequence, String? slug, String? config, DateTime? deletedAt, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
  ...
  static Future<PcoCollection<PcoPeopleFieldDefinition>> get( {String? id, PcoPeopleFieldDefinitionQuery? query, bool includeAll = false, bool includeFieldOptions = false, bool includeTab = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleFieldDefinition>> getFromFieldData(String fieldDataId, {PcoPeopleFieldDefinitionQuery? query, bool includeAll = false, bool includeFieldOptions = false, bool includeTab = false,}) async {
    query ??= PcoPeopleFieldDefinitionQuery();
    if (includeAll) query.include.addAll(PcoPeopleFieldDefinition.canInclude);
    if (includeFieldOptions) query.include.add('field_options');
    if (includeTab) query.include.add('tab');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleFieldDefinition>> getFromTab(String tabId, {String? id, PcoPeopleFieldDefinitionQuery? query, bool includeAll = false, bool includeFieldOptions = false, bool includeTab = false,}) async {
    query ??= PcoPeopleFieldDefinitionQuery();
    if (includeAll) query.include.addAll(PcoPeopleFieldDefinition.canInclude);
    if (includeFieldOptions) query.include.add('field_options');
    if (includeTab) query.include.add('tab');
  ...
  Future<PcoCollection<PcoPeopleFieldOption>> getFieldOptions({PcoPeopleFieldOptionQuery? query}) async {
