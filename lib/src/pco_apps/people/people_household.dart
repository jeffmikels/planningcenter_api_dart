...
class PcoPeopleHousehold extends PcoResource {
  ...
  /// using a path like this: `/people/v2/households/[id]`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoPeopleHousehold?> getSingle(
    String id, {
    PcoPeopleHouseholdQuery? query,
    bool includePeople = false,
  }) async {
    ...
    var retval = await PcoCollection.fromApiCall<PcoPeopleHousehold>(url,
        query: query, apiVersion: kApiVersion);
    return retval.items.isEmpty ? null : retval.items.first;
  }

  /// Will get a [PcoCollection] containing ALL [PcoPeopleHousehold] objects (expecting many)
  /// using a path like this: `/people/v2/households`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  ...
  /// Will get a single [PcoPeopleHousehold] object
  /// using a path like this: `/people/v2/people/$personId/households/[id]`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoPeopleHousehold?> getSingleFromPerson(
    String personId,
    String id, {
    PcoPeopleHouseholdQuery? query,
    bool includePeople = false,
    ...
    if (includePeople) query.include.add('people');
    var url = '/people/v2/people/$personId/households/$id';
    var retval = await PcoCollection.fromApiCall<PcoPeopleHousehold>(url,
        query: query, apiVersion: kApiVersion);
    return retval.items.isEmpty ? null : retval.items.first;
  }
  ...
  ///
  /// This function forces the `query.getAll` to be true.
  static Future<PcoCollection<PcoPeopleHousehold>> getAllFromPerson(
    String personId, {
    String? id,
    PcoPeopleHouseholdQuery? query,
    bool includePeople = false,
