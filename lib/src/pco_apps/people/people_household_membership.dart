/// =========================================================================
/// AUTO-GENERATED FILE CREATED ON 2022-10-20T17:42:51.407377
...
class PcoPeopleHouseholdMembershipQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoPeopleHouseholdMembershipOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoPeopleHouseholdMembershipFilter, String> _filterMap = {
    
  };
  static String filterString(PcoPeopleHouseholdMembershipFilter filter) => _filterMap[filter]!;
  ...
  PcoPeopleHouseholdMembershipQuery({
    ...
    if (wherePending != null) where.add(PlanningCenterApiWhere.parse('pending', wherePending));
    if (wherePersonName != null) where.add(PlanningCenterApiWhere.parse('person_name', wherePersonName));
...
class PcoPeopleHouseholdMembership extends PcoResource {
  ...
  static Future<PcoCollection<PcoPeopleHouseholdMembership>> getFromHousehold(String householdId, {String? id, PcoPeopleHouseholdMembershipQuery? query, bool includeAll = false, bool includeHousehold = false, bool includePerson = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleHouseholdMembership>> getFromPerson(String personId, {String? id, PcoPeopleHouseholdMembershipQuery? query, bool includeAll = false, bool includeHousehold = false, bool includePerson = false,}) async {
    query ??= PcoPeopleHouseholdMembershipQuery();
    if (includeAll) query.include.addAll(PcoPeopleHouseholdMembership.canInclude);
    if (includeHousehold) query.include.add('household');
    if (includePerson) query.include.add('person');
