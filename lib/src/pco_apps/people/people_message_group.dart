...
/// - `uuid` -> `?order=uuid`
enum PcoPeopleMessageGroupOrder { createdAt, fromAddress, messageCount, messageType, subject, systemMessage, uuid }
...
class PcoPeopleMessageGroupQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoPeopleMessageGroupOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoPeopleMessageGroupFilter, String> _filterMap = {
    
  };
  static String filterString(PcoPeopleMessageGroupFilter filter) => _filterMap[filter]!;
  ...
  PcoPeopleMessageGroupQuery({
    ...
    if (whereMessageCount != null) where.add(PlanningCenterApiWhere.parse('message_count', whereMessageCount));
    if (whereMessageType != null) where.add(PlanningCenterApiWhere.parse('message_type', whereMessageType));
    if (whereSubject != null) where.add(PlanningCenterApiWhere.parse('subject', whereSubject));
    if (whereSystemMessage != null) where.add(PlanningCenterApiWhere.parse('system_message', whereSystemMessage));
    if (whereUuid != null) where.add(PlanningCenterApiWhere.parse('uuid', whereUuid));
...
class PcoPeopleMessageGroup extends PcoResource {
  ...
  static Future<PcoCollection<PcoPeopleMessageGroup>> get( {String? id, PcoPeopleMessageGroupQuery? query, bool includeAll = false, bool includeApp = false, bool includeFrom = false, bool includeMessages = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleMessageGroup>> getFromMessage(String messageId, {String? id, PcoPeopleMessageGroupQuery? query, bool includeAll = false, bool includeApp = false, bool includeFrom = false, bool includeMessages = false,}) async {
    query ??= PcoPeopleMessageGroupQuery();
    if (includeAll) query.include.addAll(PcoPeopleMessageGroup.canInclude);
    if (includeApp) query.include.add('app');
    if (includeFrom) query.include.add('from');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleMessageGroup>> getFromPerson(String personId, {String? id, PcoPeopleMessageGroupQuery? query, bool includeAll = false, bool includeApp = false, bool includeFrom = false, bool includeMessages = false,}) async {
    query ??= PcoPeopleMessageGroupQuery();
    if (includeAll) query.include.addAll(PcoPeopleMessageGroup.canInclude);
    if (includeApp) query.include.add('app');
    if (includeFrom) query.include.add('from');
  ...
  Future<PcoCollection<PcoPeoplePerson>> getFrom({PcoPeoplePersonQuery? query}) async {
