...
class PcoPeopleMessageGroup extends PcoResource {
  ...
  List<T> includedFrom<T extends PcoResource>() => relationships['from']?.cast<T>() ?? [];
  List<T> includedMessages<T extends PcoResource>() => relationships['messages']?.cast<T>() ?? [];
  ...
  static Future<PcoCollection<PcoPeopleMessageGroup>> get( {String? id, PlanningCenterApiQuery? query, bool allIncludes = false}) async {
  ...
  static Future<PcoCollection<PcoPeopleMessageGroup>> getFromMessage(String messageId, {String? id, PlanningCenterApiQuery? query, bool allIncludes = false}) async {
    query ??= PlanningCenterApiQuery();
    if (allIncludes) query.include = PcoPeopleMessageGroup.canInclude;
  ...
  static Future<PcoCollection<PcoPeopleMessageGroup>> getFromPeople(String peopleId, {String? id, PlanningCenterApiQuery? query, bool allIncludes = false}) async {
    query ??= PlanningCenterApiQuery();
    if (allIncludes) query.include = PcoPeopleMessageGroup.canInclude;
  ...
  Future<PcoCollection<PcoPeoplePerson>> getFrom({PlanningCenterApiQuery? query, bool allIncludes = false}) async {
    query ??= PlanningCenterApiQuery();
