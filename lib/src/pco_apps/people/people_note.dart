...
enum PcoPeopleNoteOrder { createdAt, displayDate, id, note, noteCategoryId, updatedAt }
...
class PcoPeopleNoteQuery extends PlanningCenterApiQuery {
  ...
  PcoPeopleNoteQuery({
    ...
    if (whereNote != null) where.add(PlanningCenterApiWhere.parse('note', whereNote));
    if (whereNoteCategoryId != null) where.add(PlanningCenterApiWhere.parse('note_category_id', whereNoteCategoryId));
...
class PcoPeopleNote extends PcoResource {
  ...
  static Future<PcoCollection<PcoPeopleNote>> get( {String? id, PcoPeopleNoteQuery? query, bool includeAll = false, bool includeCategory = false, bool includeCreatedBy = false, bool includePerson = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleNote>> getFromPerson(String personId, {String? id, PcoPeopleNoteQuery? query, bool includeAll = false, bool includeCategory = false, bool includeCreatedBy = false, bool includePerson = false,}) async {
    query ??= PcoPeopleNoteQuery();
    if (includeAll) query.include.addAll(PcoPeopleNote.canInclude);
    if (includeCategory) query.include.add('category');
    if (includeCreatedBy) query.include.add('created_by');
  ...
  Future<PcoCollection<PcoPeopleNoteCategory>> getCategory({PcoPeopleNoteCategoryQuery? query}) async {
