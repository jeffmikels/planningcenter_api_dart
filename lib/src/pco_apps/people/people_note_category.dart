...
class PcoPeopleNoteCategoryQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoPeopleNoteCategoryOrder order, {bool reverse = false}) =>
  ...
  PcoPeopleNoteCategoryQuery({
    ...
    if (whereLocked != null) where.add(PlanningCenterApiWhere.parse('locked', whereLocked));
    if (whereName != null) where.add(PlanningCenterApiWhere.parse('name', whereName));
    if (whereOrganizationId != null) where.add(PlanningCenterApiWhere.parse('organization_id', whereOrganizationId));
...
class PcoPeopleNoteCategory extends PcoResource {
  ...
  /// The code generator could not automatically determine the resource type of this relationship.
  /// For type safe code, you should specify it in the type argument when calling.
  List<T> includedSubscribers<T extends PcoResource>() => (relationships['subscribers'] as List?)?.cast<T>() ?? [];
  
  /// The code generator could not automatically determine the resource type of this relationship.
  /// For type safe code, you should specify it in the type argument when calling.
  List<T> includedSubscriptions<T extends PcoResource>() => (relationships['subscriptions'] as List?)?.cast<T>() ?? [];
  

  // Class Constructors
  PcoPeopleNoteCategory.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoPeopleNoteCategory({String? id, String? name, bool? isLocked, DateTime? createdAt, DateTime? updatedAt, String? organizationId, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
  ...
  static Future<PcoCollection<PcoPeopleNoteCategory>> get( {String? id, PcoPeopleNoteCategoryQuery? query, bool includeAll = false, bool includeShares = false, bool includeSubscribers = false, bool includeSubscriptions = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleNoteCategory>> getCategoryFromNote(String noteId, {PcoPeopleNoteCategoryQuery? query, bool includeAll = false, bool includeShares = false, bool includeSubscribers = false, bool includeSubscriptions = false,}) async {
    query ??= PcoPeopleNoteCategoryQuery();
    if (includeAll) query.include.addAll(PcoPeopleNoteCategory.canInclude);
    if (includeShares) query.include.add('shares');
    if (includeSubscribers) query.include.add('subscribers');
  ...
  Future<PcoCollection<PcoPeopleNoteCategoryShare>> getShares({PcoPeopleNoteCategoryShareQuery? query}) async {
  ...
  Future<PcoCollection<PcoPeoplePerson>> getSubscribers({PcoPeoplePersonQuery? query}) async {
  ...
  Future<PcoCollection<PcoPeopleNoteCategorySubscription>> getSubscriptions({PcoPeopleNoteCategorySubscriptionQuery? query}) async {
