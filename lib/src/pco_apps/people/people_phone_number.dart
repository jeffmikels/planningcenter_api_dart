...
enum PcoPeoplePhoneNumberOrder { carrier, createdAt, location, number, primary, updatedAt }
...
class PcoPeoplePhoneNumberQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoPeoplePhoneNumberOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoPeoplePhoneNumberFilter, String> _filterMap = {
    
  };
  static String filterString(PcoPeoplePhoneNumberFilter filter) => _filterMap[filter]!;
  ...
  PcoPeoplePhoneNumberQuery({
    ...
    if (whereCarrier != null) where.add(PlanningCenterApiWhere.parse('carrier', whereCarrier));
    ...
    if (whereNumber != null) where.add(PlanningCenterApiWhere.parse('number', whereNumber));
...
class PcoPeoplePhoneNumber extends PcoResource {
  ...
  static Future<PcoCollection<PcoPeoplePhoneNumber>> getFromPerson(String personId, {String? id, PcoPeoplePhoneNumberQuery? query, }) async {
