...
class PcoPeopleTab extends PcoResource {
  ...
  List<PcoPeopleFieldDefinition> get includedFieldDefinitions => (relationships['field_definitions'] as List?)?.cast<PcoPeopleFieldDefinition>() ?? [];
  List<PcoPeopleFieldOption> get includedFieldOptions => (relationships['field_options'] as List?)?.cast<PcoPeopleFieldOption>() ?? [];

  // Class Constructors
  PcoPeopleTab.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  static Future<PcoCollection<PcoPeopleTab>> get( {String? id, PcoPeopleTabQuery? query, bool includeAll = false, bool includeFieldDefinitions = false, bool includeFieldOptions = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleTab>> getFromFieldData(String fieldDataId, {String? id, PcoPeopleTabQuery? query, bool includeAll = false, bool includeFieldDefinitions = false, bool includeFieldOptions = false,}) async {
    query ??= PcoPeopleTabQuery();
    if (includeAll) query.include.addAll(PcoPeopleTab.canInclude);
    if (includeFieldDefinitions) query.include.add('field_definitions');
    if (includeFieldOptions) query.include.add('field_options');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoPeopleTab>> getFromFieldDefinition(String fieldDefinitionId, {String? id, PcoPeopleTabQuery? query, bool includeAll = false, bool includeFieldDefinitions = false, bool includeFieldOptions = false,}) async {
    query ??= PcoPeopleTabQuery();
    if (includeAll) query.include.addAll(PcoPeopleTab.canInclude);
    if (includeFieldDefinitions) query.include.add('field_definitions');
    if (includeFieldOptions) query.include.add('field_options');
