/// =========================================================================
/// AUTO-GENERATED FILE CREATED ON 2022-06-13T18:17:42.606534
...
class PcoServicesItem extends PcoResource {
  ...
  List<T> includedArrangement<T extends PcoResource>() => relationships['arrangement']?.cast<T>() ?? [];
  List<T> includedItemNotes<T extends PcoResource>() => relationships['item_notes']?.cast<T>() ?? [];
  List<T> includedItemTimes<T extends PcoResource>() => relationships['item_times']?.cast<T>() ?? [];
  List<T> includedKey<T extends PcoResource>() => relationships['key']?.cast<T>() ?? [];
  List<T> includedMedia<T extends PcoResource>() => relationships['media']?.cast<T>() ?? [];
  List<T> includedSelectedAttachment<T extends PcoResource>() => relationships['selected_attachment']?.cast<T>() ?? [];
  List<T> includedSong<T extends PcoResource>() => relationships['song']?.cast<T>() ?? [];
