/// =========================================================================
/// AUTO-GENERATED FILE CREATED ON 2022-10-20T17:42:11.957671
...
class PcoServicesPersonTeamPositionAssignmentQuery extends PlanningCenterApiQuery {
  static final Map<PcoServicesPersonTeamPositionAssignmentOrder, String> _orderMap = {
  ...
  static String orderString(PcoServicesPersonTeamPositionAssignmentOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoServicesPersonTeamPositionAssignmentFilter, String> _filterMap = {
    PcoServicesPersonTeamPositionAssignmentFilter.timePreferenceOptions: 'time_preference_options',
  };
  static String filterString(PcoServicesPersonTeamPositionAssignmentFilter filter) => _filterMap[filter]!;
...
class PcoServicesPersonTeamPositionAssignment extends PcoResource {
  ...
  set schedulePreference(String? x) => (x == null) ? _attributes.remove(kSchedulePreference) : _attributes[kSchedulePreference] = x;
  ...
  ///
  /// pass `null` to remove key from attributes
  set preferredWeeks(List? x) => (x == null) ? _attributes.remove(kPreferredWeeks) : _attributes[kPreferredWeeks] = x;
  
  // additional setters / getters for create/update attributes
  ///
  /// pass `null` to remove key from attributes
  set timePreferenceOptionIds(String? x) => (x == null) ? _attributes.remove(kTimePreferenceOptionIds) : _attributes[kTimePreferenceOptionIds] = x;
  String get timePreferenceOptionIds => _attributes[kTimePreferenceOptionIds] ?? '';
  ///
  /// pass `null` to remove key from attributes
  ...
  PcoServicesTeamPosition? get includedTeamPosition => _firstOrNull<PcoServicesTeamPosition>(relationships['team_position']);

  // Class Constructors
  PcoServicesPersonTeamPositionAssignment.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoServicesPersonTeamPositionAssignment({required String serviceTypeId, required String teamPositionId, String? id, DateTime? createdAt, DateTime? updatedAt, String? schedulePreference, List? preferredWeeks, String? timePreferenceOptionIds, String? personId, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
    ...
    if (schedulePreference != null) obj._attributes['schedule_preference'] = schedulePreference;
    if (preferredWeeks != null) obj._attributes['preferred_weeks'] = preferredWeeks;
    if (timePreferenceOptionIds != null) obj._attributes['time_preference_option_ids'] = timePreferenceOptionIds;
  ...
  static Future<PcoCollection<PcoServicesPersonTeamPositionAssignment>> getFromPerson(String personId, {String? id, PcoServicesPersonTeamPositionAssignmentQuery? query, bool includeAll = false, bool includePerson = false, bool includeTeamPosition = false,}) async {
    query ??= PcoServicesPersonTeamPositionAssignmentQuery();
    if (includeAll) query.include.addAll(PcoServicesPersonTeamPositionAssignment.canInclude);
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoServicesPersonTeamPositionAssignment>> getFromTeam(String teamId, {String? id, PcoServicesPersonTeamPositionAssignmentQuery? query, bool includeAll = false, bool includePerson = false, bool includeTeamPosition = false,}) async {
    query ??= PcoServicesPersonTeamPositionAssignmentQuery();
    if (includeAll) query.include.addAll(PcoServicesPersonTeamPositionAssignment.canInclude);
    if (includePerson) query.include.add('person');
    if (includeTeamPosition) query.include.add('team_position');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoServicesPersonTeamPositionAssignment>> getFromServiceTypeAndTeamPosition(String serviceTypeId,String teamPositionId, {String? id, PcoServicesPersonTeamPositionAssignmentQuery? query, bool includeAll = false, bool includePerson = false, bool includeTeamPosition = false,}) async {
    query ??= PcoServicesPersonTeamPositionAssignmentQuery();
    if (includeAll) query.include.addAll(PcoServicesPersonTeamPositionAssignment.canInclude);
    if (includePerson) query.include.add('person');
    if (includeTeamPosition) query.include.add('team_position');
  ...
  Future<PcoCollection<PcoServicesPerson>> getPerson({PcoServicesPersonQuery? query}) async {
  ...
  Future<PcoCollection<PcoServicesTeamPosition>> getTeamPosition({PcoServicesTeamPositionQuery? query}) async {
