...
class PcoServicesPlanQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoServicesPlanOrder order,
          {bool reverse = false}) =>
  ...
  static String filterString(PcoServicesPlanFilter filter) =>
      _filterMap[filter]!;
...
class PcoServicesPlan extends PcoResource {
  ...
  /// using a path like this: `/services/v2/series/$seriesId/plans/[id]`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoServicesPlan?> getSingleFromSeries(
    String seriesId,
    String id, {
    PcoServicesPlanQuery? query,
    bool includeAllRelated = false,
  ...
  /// Will get a [PcoCollection] containing ALL [PcoServicesPlan] objects (expecting many)
  /// using a path like this: `/services/v2/series/$seriesId/plans`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  ...
  static Future<PcoCollection<PcoServicesPlan>> getAllFromSeries(
    String seriesId, {
    String? id,
    PcoServicesPlanQuery? query,
    bool includeAllRelated = false,
  ...
  Future<PcoCollection<PcoServicesNeededPosition>> getNeededPositions(
      {PcoServicesNeededPositionQuery? query}) async {
