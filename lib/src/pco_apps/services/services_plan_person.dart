...
class PcoServicesPlanPersonQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoServicesPlanPersonOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoServicesPlanPersonFilter, String> _filterMap = {
    
  };
  static String filterString(PcoServicesPlanPersonFilter filter) => _filterMap[filter]!;
  ...
  PcoServicesPlanPersonQuery({
    ...
    if (includeAll || includeDeclinedPlanTimes) include.add('declined_plan_times');
...
class PcoServicesPlanPerson extends PcoResource {
  ...
  static Future<PcoCollection<PcoServicesPlanPerson>> getFromPerson(String personId, {String? id, PcoServicesPlanPersonQuery? query, bool includeAll = false, bool includeDeclinedPlanTimes = false, bool includePerson = false, bool includePlan = false, bool includeTeam = false,}) async {
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoServicesPlanPerson>> getTeamMembersFromServiceTypeAndPlan(String serviceTypeId,String planId, {PcoServicesPlanPersonQuery? query, bool includeAll = false, bool includeDeclinedPlanTimes = false, bool includePerson = false, bool includePlan = false, bool includeTeam = false,}) async {
    query ??= PcoServicesPlanPersonQuery();
    if (includeAll) query.include.addAll(PcoServicesPlanPerson.canInclude);
    if (includeDeclinedPlanTimes) query.include.add('declined_plan_times');
    if (includePerson) query.include.add('person');
  ...
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoCollection<PcoServicesPlanPerson>> getFromTeam(String teamId, {String? id, PcoServicesPlanPersonQuery? query, bool includeAll = false, bool includeDeclinedPlanTimes = false, bool includePerson = false, bool includePlan = false, bool includeTeam = false,}) async {
    query ??= PcoServicesPlanPersonQuery();
    if (includeAll) query.include.addAll(PcoServicesPlanPerson.canInclude);
    if (includeDeclinedPlanTimes) query.include.add('declined_plan_times');
    if (includePerson) query.include.add('person');
  ...
  Future<PcoCollection<PcoServicesPlan>> getPlan({PcoServicesPlanQuery? query}) async {
  ...
  Future<PcoCollection<PcoServicesPlanPersonTime>> getPlanPersonTimes({PcoServicesPlanPersonTimeQuery? query}) async {
