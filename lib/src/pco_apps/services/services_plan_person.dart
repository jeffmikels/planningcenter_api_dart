...
class PcoServicesPlanPerson extends PcoResource {
  ...
  List<T> includedDeclinedPlanTimes<T extends PcoResource>() => relationships['declined_plan_times']?.cast<T>() ?? [];
  List<T> includedPerson<T extends PcoResource>() => relationships['person']?.cast<T>() ?? [];
  List<T> includedPlan<T extends PcoResource>() => relationships['plan']?.cast<T>() ?? [];
