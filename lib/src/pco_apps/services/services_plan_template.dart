...
enum PcoServicesPlanTemplateOrder {
  createdAt,
  itemCount,
  name,
  noteCount,
...
class PcoServicesPlanTemplateQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoServicesPlanTemplateOrder order,
          {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoServicesPlanTemplateFilter, String> _filterMap = {};
  static String filterString(PcoServicesPlanTemplateFilter filter) =>
      _filterMap[filter]!;
...
class PcoServicesPlanTemplate extends PcoResource {
  ...
  /// using a path like this: `/services/v2/service_types/$serviceTypeId/plan_templates/[id]`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  static Future<PcoServicesPlanTemplate?> getSingleFromServiceType(
    String serviceTypeId,
    String id, {
    PcoServicesPlanTemplateQuery? query,
  }) async {
    ...
    var retval = await PcoCollection.fromApiCall<PcoServicesPlanTemplate>(url,
        query: query, apiVersion: kApiVersion);
    return retval.items.isEmpty ? null : retval.items.first;
  }

  /// Will get a [PcoCollection] containing ALL [PcoServicesPlanTemplate] objects (expecting many)
  /// using a path like this: `/services/v2/service_types/$serviceTypeId/plan_templates`
  ///
  /// Additional options may be specified by using the `query` argument, but some
  /// query options are also available as boolean flags in this function call too.
  ...
  Future<PcoCollection<PcoServicesPlanNote>> getNotes(
      {PcoServicesPlanNoteQuery? query}) async {
