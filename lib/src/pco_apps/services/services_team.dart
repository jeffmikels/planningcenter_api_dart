...
class PcoServicesTeam extends PcoResource {
  ...
  List<T> includedPersonTeamPositionAssignments<T extends PcoResource>() => relationships['person_team_position_assignments']?.cast<T>() ?? [];
  List<T> includedServiceType<T extends PcoResource>() => relationships['service_type']?.cast<T>() ?? [];
  List<T> includedTeamLeaders<T extends PcoResource>() => relationships['team_leaders']?.cast<T>() ?? [];
  List<T> includedTeamPositions<T extends PcoResource>() => relationships['team_positions']?.cast<T>() ?? [];
