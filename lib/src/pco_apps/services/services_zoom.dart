...
class PcoServicesZoomQuery extends PlanningCenterApiQuery {
  ...
  static String orderString(PcoServicesZoomOrder order, {bool reverse = false}) =>
      (reverse ? '-' : '') + _orderMap[order]!;

  static final Map<PcoServicesZoomFilter, String> _filterMap = {
    
  };
  static String filterString(PcoServicesZoomFilter filter) => _filterMap[filter]!;
...
class PcoServicesZoom extends PcoResource {
  ...
  set aspectRatio(double? x) => (x == null) ? _attributes.remove(kAspectRatio) : _attributes[kAspectRatio] = x;
  
  /// The percentage of the zoom. Must be a value between 1.0 and 5.0
  ///
  /// pass `null` to remove key from attributes
  set zoomLevel(double? x) => (x == null) ? _attributes.remove(kZoomLevel) : _attributes[kZoomLevel] = x;
  
  /// The percentage of the document's width the zoomed document should be offset by horizontally.
  ///
  /// pass `null` to remove key from attributes
  set xOffset(double? x) => (x == null) ? _attributes.remove(kXOffset) : _attributes[kXOffset] = x;
  
  /// The percentage of the document's height the zoomed document should be offset by vertically.
  ///
  /// pass `null` to remove key from attributes
  set yOffset(double? x) => (x == null) ? _attributes.remove(kYOffset) : _attributes[kYOffset] = x;

  // Class Constructors
  PcoServicesZoom.fromJson(Map<String, dynamic> data, {List<Map<String, dynamic>>? withIncluded}): super.fromJson(kPcoApplication, kTypeString, data, withIncluded: withIncluded);
  ...
  factory PcoServicesZoom({required String attachmentId, String? id, double? aspectRatio, double? zoomLevel, double? xOffset, double? yOffset, Map<String, List<PcoResource>>? withRelationships, List<PcoResource>? withIncluded }) {
  ...
  static Future<PcoCollection<PcoServicesZoom>> getFromAttachment(String attachmentId, {String? id, PcoServicesZoomQuery? query, }) async {
